...
def load_data(asset_type):
    ...
    if asset_type == "BTC/SPX Ratio":
        ...
        # Alinhamento pelo índice (data) e cálculo do Ratio
        df = btc[['Close']].join(spx[['Close']], lsuffix='_btc', rsuffix='_spx').ffill().dropna()
        df['Price'] = df['Close_btc'] / df['Close_spx']
        df = df.reset_index() # Garante que a data vire uma coluna chamada 'Date'
    else:
...
# --- ABA 2: MVRV ---
    ...
    fig.add_trace(go.Scatter(x=df['Date_Clean'], y=df['Price'] * s, name="Market Cap", line=dict(color='white', width=1), yaxis="y2"))
